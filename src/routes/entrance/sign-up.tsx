...
import IslandSignUpForm from '../../islands/entrance/sign-up/form.tsx'
import IslandSignUpHeader from '../../islands/entrance/sign-up/header.tsx'
...
export const handler: Handlers<AfterServerDataSchema> = {
	...
	async GET(request: Request, ctx: FreshContext): Promise<Response> {
		...
		const action: string = serverData.code === 404 ? '/api/v0/entrance/sign-up' : '/api/v0/entrance/sign-up'
...
export default function SignUp({ data }: PageProps<AfterServerDataSchema>): JSX.Element {
	switch (data.code) {
		case 404:
		case 429: {
				...
				<main class='sign-up'>
					<section class='sign-up'>
						<div class='wrapper'>
							...
							<form method='POST' action={`/api/v0/entrance/sign-up?resend=${data.data?.ulid}`}>
