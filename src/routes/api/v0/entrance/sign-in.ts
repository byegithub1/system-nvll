...
export const handler: Handlers = {
	...
	async POST(_request: Request, ctx: FreshContext): Promise<Response> {
		...
		const resend: string = ctx.state.resend as string

		try {
			const result: ServerData = await processEmail(SystemKv.id(ctx), email, resend)
 ...
 * @param {string} resend - ULID Whether to resend the verification email.
 * @return {Promise<string>} A promise that resolves to a JSON string containing the token.
 */
async function processEmail(system_id: string, email: string, resend: string): Promise<ServerData> {
	...
	return sendData(true, 202, `+OK ${resend && typeof resend === 'string' ? "we've resent it" : 'verification email has been sent'}`)
}
